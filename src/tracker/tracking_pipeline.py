...
from src.tracker.field_detector import FieldTracker
...
class VideoReader:
    ...
    def get_frame_rate(self):
        if self.frame_rate is None:
            raise Exception(
                "To get the video frame rate, you must call the frames() method first: the parameter half will determine its value."
            )
        return self.frame_rate

    def frames(
        self, nb_frames: int = None, size: Tuple[int] = (640, 480), half: bool = False
    ):
        """
        ...
        while retval and (nb_frames is None or count < nb_frames):
            ...
            yield cv2.resize(image, size, interpolation=cv2.INTER_AREA), count * (
                1 + half
            ) / self.frame_rate
            count += 1
            if half:
...
def track_pipeline(from_cam=True, vis=False, check_field_every=50, batch_size=25):
    ...
    for i, [img, timestamp] in enumerate(f):
        ...
        cropped_img, shift_x, shift_y = fd.mask_and_crop_field(
            img, corners, expand_by=25
        )

        rolling_buffer[i % batch_size] = (
            img,
            cropped_img,
            shift_x,
            shift_y,
        ...
        if (i + 1) % batch_size == 0:
            players_batch = player_tracker.track_batch(
                [img[1][..., ::-1] for img in rolling_buffer]
            )  # BGR TO RGB
            ...
            for idx, [img, cropped_img, s_x, s_y, corn] in enumerate(rolling_buffer):
                ...
                for xmin, ymin, xmax, ymax, conf, n_class in players_batch[idx]:
                    if vis:
                        img = cv2.rectangle(
                            img,
                            (int(xmin) + s_x, int(ymin) + s_y),
                            (int(xmax) + s_x, int(ymax) + s_y),
                            color=(0, 0, 255) if n_class == 0 else (255, 0, 0),
                            thickness=1,
                        )
                    if n_class == 0:
                        sending_buffer[idx]["red_players"].append(
                            (
                                ((xmin + xmax) / 2 + s_x) / img.shape[0],
                                ((ymin + ymax) / 2 + s_y) / img.shape[1],
                                (xmax - xmin) / img.shape[0],
                                (ymax - ymin) / img.shape[1],
                            )
                        )
                    else:
                        sending_buffer[idx]["blue_players"].append(
                            (
                                ((xmin + xmax) / 2 + s_x) / img.shape[0],
                                ((ymin + ymax) / 2 + s_y) / img.shape[1],
                                (xmax - xmin) / img.shape[0],
                ...
                if vis:
                    cv2.polylines(
                        img, [np.array(corn)], True, color=(0, 255, 0), thickness=3
                    )
                ...
                if vis:
                    cv2.imwrite(
                        os.path.join(
                            dir_out, f"{recording_name}_{i-batch_size+idx+1}.jpg"
                        ),
                        img,
                    )
